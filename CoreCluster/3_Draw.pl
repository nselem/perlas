...
#	$color1 -=int( $scale*(1 -$percent/100.0 ));
#	 $color2 -=int( $scale*(1 -$percent/100.0 ));
#	 $color3 -=int( $scale*(1 -$percent/100.0 ));
...
##label
  ...
  my $desc="Colores".$opacity." ".$orgNum." ".$org." ".$orgName." ".$start." ".$end." ".$dir." ".$id_peg." ".$func;
...
foreach my $context(@CLUSTERS){
		...
		#	print "start $start stop $stop\n";
			...
			print "Key Start $start->$s1, stop $stop->$e1, dir $dir, \n";
			if($dir eq '+'){
			$ARROWS{$s1}=[$s1,$e1,$key,$dir,$color,$DirCont,$w,$e0,$func,$id_peg,$percent];
					}
			else{
			$ARROWS{$e1}=[$s1,$e1,$key,$dir,$color,$DirCont,$w,$e0,$func,$id_peg,$percent];
					}
		}
		...
		my $level=1; my $count=0; my $lastStop=0;	
			...
			arrow($s,$t,$tv,$grueso,$s1,$e1,$cont_number,$dir,$color,$level,$DirCont,$w,$e0,$func,$id_peg,$orgName,$cutleft,$refYCOORD,$refColorNames,$key,$percent);
