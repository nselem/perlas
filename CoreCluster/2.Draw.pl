...
##label
  ...
  my $desc=$orgNum." ".$org." ".$orgName." ".$start." ".$end." ".$dir." ".$id_peg." ".$func;
...
my $cont_number=0;
	foreach my $key(sort { $a <=> $b } keys %{$refCONTEXTS}){  ###For each cluster
		...
		my $level=1; my $count=0; my $lastStop=0;	
			...
			arrow($s,$t,$tv,$grueso,$s1,$e1,$cont_number,$dir,$color,$level,$DirCont,$w,$e0,$func,$id_peg,$orgName,$cutleft,$refYCOORD,$refColorNames,$key);
...
sub Fill_Colors{
	...
	for(my $i=1;$i<=100;$i++){
	my $color1=int(rand(250));	
	my $color2=100+int(rand(150));	
	my $color3=int(rand(250));	
