...
GetOptions('help|?' => \$help,'verbose' => \$verbose,'prefix=s' => \$prefix,'dir=s' => \$dir);
#####################################################################################################
...
my $out_directory = "/home/fbg/lab/ana/RASTtk/$infile/CONCATENADOS"; 
...
#### Voy a abrir todos los archivos del directorio para llenar el arreglo files
     ...
     opendir (DIR, $directory) or die $!;  ### Abriendo el directorio
        ...
        #                 if (($file=~m/^\d/)&&($file=~m/nucleotido/)){ ## linea para nucleotidos (Descomentar)
