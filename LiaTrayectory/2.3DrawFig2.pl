...
sub Line{
	...
	if ($flagPRO==1 and $flagPRA==0){
		if ($refPRO->{$final}>$refPRO->{$inicio}){ ## Drawing trayectory lines
			$svg->line(x1 => $x1, y1 => $y1, x2 => $x2, y2 => $y2, 
			style => {'stroke' => "rgb(150,0,0)",'stroke-width' =>1.5,
...
if ($variant>9){
$svg->text( x  => $x-10, y  => $y+5, 'font-size'=>"10",'font-family'=>'Arial')->cdata("$mut.$variant"); 
}
else{ ## if only ne digit for variant I move a little to center on x coordinate
$svg->text( x  => $x-7, y  => $y+5, 'font-size'=>"10",'font-family'=>'Arial')->cdata("$mut.$variant"); 
}
####### activity circles
