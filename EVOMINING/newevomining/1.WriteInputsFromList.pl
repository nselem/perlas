...
########3 Nota neceisto corregir que lea la cantidad total de genes del archivo total de base de datos de genomas.
## This script generates one images with all the ids on file

#print "Number of contexts $NumberContexts\n";
my $fileNames="RAST.ids"; ##File with genome ids RASt.ids
my $user=shift;
my @gen_id=@ARGV; ##File with genome ids to get the contexts
my $OutDir="INPUT_$user";
#print "Reading $file\n";
my %LIST;
my %NAMES;
my $long=80000;
...
###################### Main ########################################
#if (-d "INPUT"){ #Consultar christian
#	`rm INPUT/*.input`;
#	`rm -rf INPUT`;
#	}
`mkdir $OutDir`;
...
print "Text contexts file have been generated\n";
######################################################################
###################### Subs ##########################################
...
sub ContextArray{
	...
	open(FILE,">$OutDir/$peg.input")or die "could not open $orgs.input file $!";
	my @CONTEXT;
