...
open (VIAS, "/var/www/newevomining/PasosBioSin/ALL_curadoHEADER.fasta");
$counter=0;
while (<VIAS>){
chomp;
  if($_ =~ />/){
...
####weekend#system "ls /var/www/newevomining/FASTASparaNP > /var/www/newevomining/ls.FASTANP";
...
chomp;
 ...
 ####weekend# open(OUTFASTA, ">/var/www/newevomining/NewFASTASparaNP/$_");
 open(TAM, "/var/www/newevomining/FASTASparaNP/$_") or die $!;
  $contDentro=0;
  $contfuera=0;
  $contfueraabajo=0;
 ...
 ###################################################
  ...
  $sumaTAM=0;
   ...
   foreach my $xids ( @arrpasos2 ){
     ...
     ####weekend#  print OUTFASTA "$idssize[0]\n$SeqRecortada\n";
       $contfuera++;
     }
...
##################weekend#####################
## version con todos##
####weekend#system "/opt/ncbi-blast-2.2.28+/bin/blastp -db /var/www/newevomining/NPDB/NP_DB_NOVEMBER2014clean.db -query /var/www/newevomining/NewFASTASparaNP/$_ -outfmt 6 -num_threads 4 -evalue $eval -out /var/www/newevomining/blast/$_\_ExpandedVsNp.blast";
...
#print "<h1>cat $_ |cut -f2 |sort -u >$_.recruitedUniq</h1>";
   ...
   if($sii){
    ##############weekend###############          system "cat $_ |cut -f2 |sort -u >$_.recruitedUniq"; #Si tiene texto el archivo extrae la colunma2 que contiene el nombre del NP
   }
}
close EXP;
...
chomp;
 ...
 $sizeFIle= -s $_;
 if($sizeFIle > 0){
 ####weekend# open(OUTFASTA,">$_.fasta") or die $!;
  $nam=$_;
...
open(NPDB, "/var/www/newevomining/NPDB/NP_DB_NOVEMBER2014clean.txt")or die $!;
 $si=0;  
 while($line2=<NPDB>){
  chomp($line2);
    if ($line2 =~ />/){
...
## version recortados
###weekend##system "cat /var/www/newevomining/NewFASTASparaNP/$tempor.fasta $x.fasta_ExpandedVsNp.blast.2.recruitedUniq.fasta> $x.concat.fasta";
...
#print "Content-type: text/html\n\n";
print qq| <form method="post" action="/cgi-bin/newevomining/alignGcontextBASIC.pl" name="forma"> |;
