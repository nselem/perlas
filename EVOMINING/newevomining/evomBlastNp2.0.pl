...
open (BNPOUT, ">$apacheCGIpath/$pidfecha/whereIdie") or die $!;
while (<BNP>){
chomp;
 ...
 open(TAM, "$pidfecha/FASTASparaNP/$_") or die $!;
  ...
  print BNPOUT "antesdel 1er while\n";
 while($lineaa=<TAM>){
 chomp($lineaa);
  if($lineaa =~ />/){
     $headerr=$lineaa;
 ...
 $cuentaSEQ=0;
 undef @arrpasos2;
  print BNPOUT "antesdel 1er foreach lasrgascortas\n";
 ...
 ###################################################
   ...
   $cuentaSEQ=0;
  $sumaTAM=0;
   print BNPOUT "antesdel 1er foreach arrpasos2\n";
...
## version con todos##
####weekend#
system "blastp -db $apacheCGIpath/NPDB/NP_DB_NOVEMBER2014clean.db -query $apacheCGIpath/$pidfecha/NewFASTASparaNP/$_ -outfmt 6 -num_threads 4 -evalue $eval -out $apacheCGIpath/$pidfecha/blast/$_\_ExpandedVsNp.blast";
