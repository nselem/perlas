...
print "<h1>Parametros recibidos: e-value:$eval -  score: $score2 </h1>\n";
...
chomp;
 ...
 # print "$datpasosBIO[1]_$datGenomas[5]\n";
  if(!exists $hashUniqGI{$datGenomas[1]}){
    if ($datblast[11] >= $score2){
     $llave="$datpasosBIO[1]_$datGenomas[5]";
     $hashGenomas{$llave}++;
     $hashGIs{$llave}=$hashGIs{$llave}."\t".$datGenomas[1];
     $hashUniqGI{$datGenomas[1]}=1;
     
     if( !exists$hashpasos{$datpasosBIO[1]}){
       $hashpasos{$co} =$datpasosBIO[1]; #quiza haya que concatenar el numero al a via cuando se agreguen las demas vias
       $co++;
     }
     print LOG "$_\n";
    }
...
<form name="activa" action="/cgi-bin/newevomining/evomBlastNp.pl"
method="post">
<p><big>Expanded enzyme families<br>
</big></p>
<table
style="text-align: left; width: 201px; height: 137px; margin-left: auto; margin-right: auto;"
border="1" cellpadding="2" cellspacing="2">
<tbody>
<tr>
<td style="vertical-align: middle; text-align: center;">
<p><big>Blast option:</big></p>
<big>e-value:</big><textarea style="width: 65px; height: 25px;"
cols="1" rows="1" name="evalue">0.0001</textarea> Minimum Score:<textarea
style="width: 50px; height: 25px;" cols="1" rows="1" name="score">150</textarea>
<p><button style="width: 70px; height: 30px;" value="Submit"
name="Submit">SUBMIT</button> </p>
</td>
</tr>
</tbody>
</table>
