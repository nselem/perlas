...
#$genomesOUTDB="NellyDaniel/TodosHEADER.fasta.db";
 ...
 $genomes="tenGenomes.fasta";
 $genomesOUT="tenGenomesHEADER.fasta";
 #$genomesOUTDB="NellyDaniel/11Oct2015HEADER.fasta.db";
 #$MEtCentral="tRAPs_EvoMining.fa";
 $listaNombres="lista10";
...
#    $_ =~ s/\.//g;
    ...
    @arr=split(/\|/,$_);
    #print "$_\n";
    print NAMESOUT "$arr[$#arr]\t";   #Legible	
    $arr[$#arr]=~ s/\.//g;
    $legible=$arr[$#arr];
    $arr[$#arr]=~ s/\_//g;
    $arr[$#arr]=~ s/ //g;
    ...
    if(!exists $hashUniq{$arr[$#arr]}){
       ...
       print HASHESOUT qq/\$hashOrdenNOMBRES{'$legible'}=$counting;-->$hashOrdenNOMBRES{$legible}\n/;
...
#print "blast...\n";
#system "blastp -db $genomesOUTDB -query ../PasosBioSin/$MEtCentral -outfmt 6 -num_threads 4 -evalue 0.0001 -out ../blast/pscpNellyDaniel240915.blast";
