...
<form method="post" action="/cgi-bin/newevomining/evomBlastNp2.0ORIG.pl"
name="foma">
  ...
  open (FAST, "$dirDB/$db") or die "$! $dirDB/$db";
     ...
     while(<FAST>){
      ...
      if($_ =~ />gi\|(\d+\.\d+\.\d+)\|.+/){
        ...
        if(exists $Allgis{$id}){
          $siH=1;
	 open (FASTA, ">>$apacheCGIpath/$outdir/FASTASparaNP/$Allgis{$id}.fasta") or die $!;
 	 print FASTA "$_\n";
	 close FASTA;
        }
        else{
