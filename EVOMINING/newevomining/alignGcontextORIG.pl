...
|;
my %hashNUM_NP='';
open (LOG, ">$outdir/entradaAlign.log");

#--------------------- Alineando---------------------------
foreach my $c (@dat){
  print LOG "$c\n";
      system "muscle -in $apacheCGIpath/$outdir/blast/$c.concat.fasta -out $apacheCGIpath/$outdir/blast/$c.aln";
  if($c ne 'Submit' and $c ne '' and  $c ne ' '){ 
      $nam="$apacheCGIpath/$outdir/blast/$c.aln";
     ###########weekend############### 
     addGap($nam);
   }   
}
...
foreach my $cc (@dat){
 ...
 #-------------------------------------------------------------------------------------------------
   system "whoami>quien";
   system "$gblockPATH/Gblocks ../$cc\gap.aln.out -b4=5 -b5=H -b3=10>LOG$cc.GBLOCks" or die$!;
  #------------------------------------------------------------------------------
 ...
 open(AGAIN, "../$cc\gap.aln.out-gb") or die $!;
 open(FINAL, ">../$cc\gap.aln.out-gb2") or die $!;
  while($linea=<AGAIN>){
    chomp($linea);
   if($linea=~ />/){
...
#---------------------hash num and NP--------------
 open(HASH, "$apacheCGIpath/$outdir/hash.log")or die $!;
   while(<HASH>){
   chomp;
      #@data=split("---", $_);
      #@gis=split(/,/, $data[1]);
...
foreach my $cc (@dat){
 ...
 chdir "$apacheCGIpath/$outdir/blast/seqf/";
  system "quicktree -in a -out t -boot 10000  $apacheCGIpath/$outdir/blast/seqf/$cc.stock > $apacheCGIpath/$outdir/blast/seqf/tree/$cc.tree";
 #########weekend######### 
 system "$newickPATH/nw_distance -n $apacheCGIpath/$outdir/blast/seqf/tree/$cc.tree >$apacheCGIpath/$outdir/blast/seqf/distance.$cc";
 $dist="distance.$cc";
 ...
 #########weekend######### 
 system "$newickPATH/nw_display -w 5600 -sr -S -v 100 -l 'font-size:x-small' -b 'visibility:hidden' -o $apacheCGIpath/$outdir/blast/seqf/tree/ornament.$cc $apacheCGIpath/$outdir/blast/seqf/tree/$cc.tree >$apacheCGIpath/$outdir/blast/seqf/tree/$cc.p.svg";
...
print qq |<div><td >tree</td></div>|;
 open(FF, "$apacheCGIpath/$outdir/hash.log"); ###aqui hay que modificar
 while ($xef = <FF>){
     chomp($xef);
     @array =split("---",$xef);
    ...
    foreach my $xc (@dat){
        if($array2[0] eq $xc){
          ...
          open(XT, "$apacheCGIpath/$outdir/blast/seqf/$onlyD") or die $onlyD;
	  ...
	  #print qq |<div><td>$array[1]</td></div>|;
	    ...
	    print qq |</table>|;
	    print qq |</td>|;
	   print qq |<div><td ><a href="http://10.10.100.156/cgi-bin/newevomining/tree2_2TOTALexecORIG.pl?$array2[0]&&$outdir" target="_blank">check tree</a><br><a href="http://10.10.100.156/newevomining/$outdir/blast/seqf/tree/$array2[0].tree" target="_blank"> Download newick_tree</a></td></div>|;
...
sub findDistance2 {
  ...
  #########weekend######### 
  open (SI,">$apacheCGIpath/$outdir/si.log");
 #########weekend#########  
 print SI "inicio $dis, $numerin\n";
  ...
  #########weekend######### 
   ...
   #########weekend#########  
   system "$newickPATH/nw_clade -c 1 $apacheCGIpath/$outdir/blast/seqf/tree/$numerin.tree $np |$newickPATH/nw_distance -n ->>$apacheCGIpath/$outdir/blast/seqf/distance.$numerin.only1";
    #print NPPS "$numerin.tree---$np\n";
   #########weekend######### 
    system "echo ==== >>$apacheCGIpath/$outdir/blast/seqf/distance.$numerin.only1";
  }
 #########weekend######### 
  close SI;
   $seccion=0;
   $colorSeccion=0;
 #########weekend#########
   open (NPPS,">$apacheCGIpath/$outdir/$numerin.NPPS.log");
 #########weekend######### 
  open (DOUT, ">$apacheCGIpath/$outdir/blast/seqf/distance.$numerin.only");
  open (D, "$apacheCGIpath/$outdir/blast/seqf/distance.$numerin.only1");
  #########weekend#########
    open (ORNAMENT, ">$apacheCGIpath/$outdir/blast/seqf/tree/ornament.$numerin");
  #########weekend#########
  ...
  close D;
  $cont=0; 
  $nomee="$apacheCGIpath/$outdir/blast/seqf/distance.$numerin.only.cssmap";
  #########weekend#########
  open (FDOUT, ">$nomee");
...
sub findDistance {
  my ($dis, $numerin)= @_;
   ...
   open (D, "$apacheCGIpath/$outdir/blast/seqf/$dis");
   while (<D>){
      chomp;
      @a=split("\t", $_);
      $hash{$a[1]}=$a[0];
