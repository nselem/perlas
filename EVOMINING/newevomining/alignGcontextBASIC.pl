...
|;
my %hashNUM_NP='';
open (LOG, ">/var/www/newevomining/entradaAlign.log");

#--------------------- Alineando---------------------------
foreach my $c (@dat){
  print LOG "$c\n";
  ###########weekend###############     system "muscle -in /var/www/newevomining/blast/$c.concat.fasta -out /var/www/newevomining/blast/$c.aln";
  if($c ne 'Submit' and $c ne '' and  $c ne ' '){ 
      $nam="/var/www/newevomining/blast/$c.aln";
     ###########weekend###############     addGap($nam);
   }   
}
close LOG;
...
foreach my $cc (@dat){
 ...
 ############weekend##############   system "/opt/Gblocks_0.91b/Gblocks ../$cc\gap.aln -b4=5 -b5=H -b3=10>LOG$cc.GBLOCks";
...
foreach my $cc (@dat){
 ...
 #chdir "/var/www/newevomining/blast/seqf/";
#############weekend#############system "/opt/hmmer-3.1b1-linux-intel-x86_64/binaries/esl-reformat --informat afa stockholm ../$cc\gap.aln-gb >$cc.stock";
...
#---------------------hash num and NP--------------
 open(HASH, "/var/www/newevomining/hash.log")or die $!;
   while(<HASH>){
   chomp;
      #@data=split("---", $_);
      #@gis=split(/,/, $data[1]);
...
foreach my $cc (@dat){
 ...
 chdir "/var/www/newevomining/$outdir/blast/seqf/";
 #########weekend#########  system "/opt/quicktree_1.1/bin/quicktree -in a -out t -boot 10000  /var/www/newevomining/blast/seqf/$cc.stock > /var/www/newevomining/blast/seqf/tree/$cc.tree";
 #########weekend######### 
 system "nw_distance -n /var/www/newevomining/blast/seqf/tree/$cc.tree >/var/www/newevomining/blast/seqf/distance.$cc";
 $dist="distance.$cc";
 ...
 #########weekend######### 
 system "nw_display -w 5600 -sr -S -v 100 -i 'font-size:xx-small' -b 'visibility:hidden' -o /var/www/newevomining/blast/seqf/tree/ornament.$cc /var/www/newevomining/blast/seqf/tree/$cc.tree >/var/www/newevomining/blast/seqf/tree/$cc.p.svg";
...
print qq |<div><td >tree</td></div>|;
 open(FF, "/var/www/newevomining/hash.log"); ###aqui hay que modificar
 while ($xef = <FF>){
     chomp($xef);
     @array =split("---",$xef);
    ...
    foreach my $xc (@dat){
        if($array2[0] eq $xc){
          ...
          open(XT, "/var/www/newevomining/blast/seqf/$onlyD") or die $onlyD;
	  ...
	  #print qq |<div><td>$array[1]</td></div>|;
	    ...
	    foreach my $x (@gisV){
	      # print qq |<tr>|;
	       #print qq |<div class="campo2"><td>$acum</td></div>|;
               print qq |<div ><a href="" target="_blank" onClick="window.open(this.href, this.target, 'width=300,height=800'); return false;">$x</a></div>|;
	    ...
	    print qq |</table>|;
	    print qq |</td>|;
	   print qq |<div><td ><a href="http://10.10.100.24/cgi-bin/newevomining/treeBASIC.pl?$array2[0]&&$outdir" target="_blank">check tree</a><br><a href="http://10.10.100.24/newevomining/blast/seqf/tree/$array2[0].tree" target="_blank"> Download newick_tree</a></td></div>|;
...
sub findDistance2 {
  ...
  #########weekend######### 
  open (SI,">/var/www/newevomining/si.log");
 #########weekend#########  
 print SI "inicio $dis, $numerin\n";
  ...
  #########weekend######### 
   ...
   #########weekend#########  
   system "nw_clade -c 1 /var/www/newevomining/blast/seqf/tree/$numerin.tree $np |nw_distance -n ->>/var/www/newevomining/blast/seqf/distance.$numerin.only1";
    #print NPPS "$numerin.tree---$np\n";
   #########weekend######### 
    system "echo ==== >>/var/www/newevomining/blast/seqf/distance.$numerin.only1";
  }
 #########weekend######### 
  close SI;
   $seccion=0;
   $colorSeccion=0;
 #########weekend#########
   open (NPPS,">/var/www/newevomining/$numerin.NPPS.log");
 #########weekend######### 
  open (DOUT, ">/var/www/newevomining/blast/seqf/distance.$numerin.only");
  open (D, "/var/www/newevomining/blast/seqf/distance.$numerin.only1");
  #########weekend#########
    open (ORNAMENT, ">/var/www/newevomining/blast/seqf/tree/ornament.$numerin");
  #########weekend#########
  ...
  close D;
  $cont=0; 
  $nomee="/var/www/newevomining/blast/seqf/distance.$numerin.only.cssmap";
  #########weekend#########
  open (FDOUT, ">$nomee");
...
sub findDistance {
  my ($dis, $numerin)= @_;
   ...
   open (D, "/var/www/newevomining/blast/seqf/$dis");
   while (<D>){
      chomp;
      @a=split("\t", $_);
      $hash{$a[1]}=$a[0];
